...
def analyze_benchmark(file_path):
    ...
    # 2. Determine Data Type (prefer int, then double, then others)
    ...
    desired_patterns = [
        ...
        "MANY_DUPLICATES_10", "MANY_DUPLICATES_50",
        "ORGAN_PIPE", "SAWTOOTH"
    ]
    ...
    # regex-like search for parallel algos
    par_16_algo = 'dual_pivot_parallel_16'
    par_24_algo = 'dual_pivot_parallel_24'
    std_algo = 'std_sort'

    # Verify which ones represent strict matches or close enough
    # If exact names are not in "all_algorithms", we might need to search
    # But user prompt implies these names.

    available_algos = sorted(list(all_algorithms))

    print("-" * 100)
    print(f"{'Pattern':<20} | {'Seq (ms)':<10} | {'Par 16 (ms)':<12} | {'Spd 16':<8} | {'Par 24 (ms)':<12} | {'Spd 24':<8} | {'std::sort':<10}")
    print("-" * 100)
    ...
    for pat in desired_patterns:
        ...
        speedup_24 = f"{t_seq / t_p24:.2f}x" if (t_seq and t_p24) else "-"

        s_std = f"{t_std:.2f}" if t_std is not None else "N/A"

        print(f"{pat:<20} | {s_seq:<10} | {s_p16:<12} | {speedup_16:<8} | {s_p24:<12} | {speedup_24:<8} | {s_std:<10}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        path = default_path
        # Check if running from root or benchmarks dir
        if not os.path.exists(path):
            alt_path = "../" + path
            if os.path.exists(alt_path):
                path = alt_path
            elif os.path.exists("results/aggregate/summary_representative.csv"):
                path = "results/aggregate/summary_representative.csv"
