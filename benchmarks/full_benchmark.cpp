...
// Logarithmically spaced array sizes (50 total)
const std::vector<size_t> ARRAY_SIZES = {
    1, 2, 3, 4, 5, 6, 8, 10, 13, 16, 21, 26, 33, 42, 54, 68, 86, 109, 138, 175, 222,
    281, 355, 449, 568, 719, 910, 1151, 1456, 1842, 2329, 2947, 3727, 4714,
    5963, 7543, 9540, 12067, 15264, 19306, 24420, 30888, 39069, 49417,
    62505, 79060, 100000
};

// Array pattern names (12 total)
const std::vector<std::string> PATTERN_NAMES = {
    "Random", "Sorted", "ReverseSorted", "NearlySorted", "ReverseNearlySorted",
    "FewUnique", "Identical", "Range", "Alternating", "MostlySmall",
    "Scalability", "Permutation"
};
...
// Type names (17 total - bool disabled due to std::vector<bool> issues)
...
template<typename T>
double runDualPivotQuicksort(std::vector<T> data) {
...
template<typename T>
double runStdSort(std::vector<T> data) {
    return BenchmarkTimer::measureTime([&]() {
...
template<typename T>
double runStdStableSort(std::vector<T> data) {
    return BenchmarkTimer::measureTime([&]() {
...
template<typename T>
double runQsort(std::vector<T> data) {
    return BenchmarkTimer::measureTime([&]() {
...
#define DISPATCH_TYPE(type_name, cpp_type) \
    if (type_name == #cpp_type) { \
        runSingleBenchmark<cpp_type>(csvFile, type_name, pattern_name, array_size); \
        return; \
    }
...
void runBenchmarkForType(std::ofstream& csvFile, const std::string& type_name, 
    ...
    // Character types
    DISPATCH_TYPE(type_name, char)
    DISPATCH_TYPE(type_name, unsigned char)
    DISPATCH_TYPE(type_name, signed char)
...
int main() {
    std::cout << "Starting Comprehensive Dual-Pivot Quicksort Benchmark" << std::endl;
    std::cout << "======================================================" << std::endl;
    std::cout << "Total test combinations: " << ALGORITHM_NAMES.size() << " algorithms × " 
              << PATTERN_NAMES.size() << " patterns × " << TYPE_NAMES.size() << " types × " 
              << ARRAY_SIZES.size() << " sizes = " 
              << (ALGORITHM_NAMES.size() * PATTERN_NAMES.size() * TYPE_NAMES.size() * ARRAY_SIZES.size()) 
              << " tests" << std::endl;
    std::cout << std::endl;
    ...
    if (!csvFile.is_open()) {
        std::cerr << "Error: Could not open benchmark_results.csv for writing" << std::endl;
        return 1;
    }
    
    // Write CSV header
    csvFile << "Algorithm,Pattern,DataType,ArraySize,ExecutionTime_ms" << std::endl;
    ...
    auto total_duration = std::chrono::duration_cast<std::chrono::seconds>(benchmark_end - benchmark_start);
    ...
    std::cout << "- Array sizes: " << ARRAY_SIZES.size() << " (from " << ARRAY_SIZES.front() 
              << " to " << ARRAY_SIZES.back() << ")" << std::endl;
