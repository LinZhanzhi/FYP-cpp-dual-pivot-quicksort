...
void dual_pivot_quicksort_impl(RandomIt first, RandomIt last, Compare comp) {
  ...
  bool different_pivots =
      !comp(pivot1, pivot2) && !comp(pivot2, pivot1) ? false : true;
  ...
  *great = pivot2;
  ...
  if (different_pivots && (static_cast<size_t>(great - less) >
                           static_cast<size_t>(len) - DIST_SIZE)) {
...
template <typename RandomIt, typename Compare>
void dual_pivot_quicksort_instrumented(
    RandomIt first, RandomIt last, Compare comp,
    SortMetrics<RandomIt, Compare> &metrics) {
